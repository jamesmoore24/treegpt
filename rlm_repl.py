...
try:
    _ctx_parsed = json.loads(CONTEXT)
    context_type = type(_ctx_parsed).__name__  # "list", "dict", etc.
...
initial_user_prompt = (
    ...
    "IMPORTANT RULES:\n"
    "1. The `context` variable already contains the document/data you need. Do NOT reassign or overwrite it.\n"
    "2. Use `llm_query(prompt)` or `llm_query_batched(prompts)` to analyze the content — never try to manually parse it with string splits.\n"
    "3. Use `print(context[:2000])` to preview the context, then use llm_query() to analyze chunks.\n\n"
...
_repl_locals: dict = {}
_repl_final: list = [None]  # set by FINAL() / FINAL_VAR() called inside repl blocks
...
def _FINAL(answer):
    """Allow the model to call FINAL() directly inside a repl block."""
    _repl_final[0] = str(answer)
    return answer


def _FINAL_VAR(var_name: str):
    """Allow the model to call FINAL_VAR() directly inside a repl block."""
    val = repl_namespace.get(var_name) or _repl_locals.get(var_name)
    if val is not None:
        _repl_final[0] = str(val)
        return val
    return f"[variable '{var_name}' not found in REPL namespace]"
...
for iteration in range(MAX_ITERATIONS):
    ...
    for code in repl_blocks:
        ...
        _repl_locals.update({
            ...
            and k not in ("context", "llm_query", "llm_query_batched", "SHOW_VARS", "FINAL", "FINAL_VAR")
        })
        output = stdout
        if stderr:
    ...
    # Check if FINAL() / FINAL_VAR() was called inside a repl block
    if _repl_final[0] is not None:
        final_answer = _repl_final[0]
        break

    # Check for FINAL / FINAL_VAR in the LLM's text response (outside code blocks)
...
# ─── Synthesis step ───────────────────────────────────────────────────────────
# Make one final sub-LM call to convert raw REPL output (Python values, partial
# data, etc.) into a clean human-readable answer for the original prompt.

def _synthesize(raw: str | None) -> str:
    MAX_RAW = 8000  # keep synthesis prompt well within context limits
    if raw:
        raw_trimmed = raw[:MAX_RAW] + ("..." if len(raw) > MAX_RAW else "")
        prompt = (
            f"Original question: {PROMPT}\n\n"
            f"Research findings:\n{raw_trimmed}\n\n"
            f"Provide a clear, complete, well-formatted answer to the original question. "
            f"If the findings contain raw data (lists, numbers, etc.), interpret them."
        )
    else:
        # No answer captured — use the last few conversation turns as context
        recent = "\n\n".join(
            f"[{m['role'].upper()}]: {m['content'][:1000]}"
            for m in conversation[-6:]
        )
        recent = recent[:MAX_RAW]
        prompt = (
            f"Original question: {PROMPT}\n\n"
            f"Recent research steps:\n{recent}\n\n"
            f"Based on the research done above, provide the best possible answer "
            f"to the original question."
        )
    return _sub_llm_call(prompt)

_push_event({"type": "synthesis_start"})
synthesized = _synthesize(final_answer)
if synthesized:
    final_answer = synthesized

# ─── Output final result ──────────────────────────────────────────────────────

result = final_answer if final_answer else "No answer could be determined."
